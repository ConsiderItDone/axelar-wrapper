...
export function writeHTTP_Request(writer: Write, type: HTTP_Request): void {
  ...
  writer.writeOptionalArray(type.headers, (writer: Write, item: Types.HTTP_Header): void => {
  ...
  writer.writeOptionalArray(type.urlParams, (writer: Write, item: Types.HTTP_UrlParam): void => {
  ...
  writer.writeOptionalString(type.body);
  writer.context().pop();
}
...
export function readHTTP_Request(reader: Read): HTTP_Request {
  ...
  while (numFields > 0) {
    ...
    if (field == "headers") {
      ...
      _headers = reader.readOptionalArray((reader: Read): Types.HTTP_Header => {
      ...
      _urlParams = reader.readOptionalArray((reader: Read): Types.HTTP_UrlParam => {
      ...
      _body = reader.readOptionalString();
      reader.context().pop();
    }
    reader.context().pop();
  }
